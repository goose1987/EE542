...
#include "SPIS_1.h"
    ...
    volatile uint8 SPIS_1_TXBUFFER[SPIS_1_TXBUFFERSIZE] = {0u};
    volatile uint8 SPIS_1_txBufferRead = 0u;
    volatile uint8 SPIS_1_txBufferWrite = 0u;
    volatile uint8 SPIS_1_txBufferFull = 0u;

#endif /* SPIS_1_TXBUFFERSIZE > 4u */

#if (SPIS_1_RXBUFFERSIZE > 4u)

    volatile uint8 SPIS_1_RXBUFFER[SPIS_1_RXBUFFERSIZE] = {0u};
    volatile uint8 SPIS_1_rxBufferRead = 0u;
    volatile uint8 SPIS_1_rxBufferWrite = 0u;
    volatile uint8 SPIS_1_rxBufferFull = 0u;
...
void SPIS_1_Init(void) 
{
    ...
    /* ISR initialization */
    #if(SPIS_1_InternalTxInterruptEnabled)

        CyIntDisable(SPIS_1_TX_ISR_NUMBER);

        /* Set the ISR to point to the SPIS_1_isr Interrupt. */
        CyIntSetVector(SPIS_1_TX_ISR_NUMBER, SPIS_1_TX_ISR);

        /* Set the priority. */
        CyIntSetPriority(SPIS_1_TX_ISR_NUMBER, SPIS_1_TX_ISR_PRIORITY);

    #endif /* SPIS_1_InternalTxInterruptEnabled */

    #if(SPIS_1_InternalRxInterruptEnabled)

        CyIntDisable(SPIS_1_RX_ISR_NUMBER);

        /* Set the ISR to point to the SPIS_1_isr Interrupt. */
        CyIntSetVector(SPIS_1_RX_ISR_NUMBER, SPIS_1_RX_ISR);
    ...
    /* Configure the Initial interrupt mask */
        ...
        SPIS_1_TX_STATUS_MASK_REG  = SPIS_1_TX_INIT_INTERRUPTS_MASK &
                                                ~SPIS_1_STS_TX_FIFO_NOT_FULL;
    #else /* SPIS_1_TXBUFFERSIZE < 4u */
        SPIS_1_TX_STATUS_MASK_REG  = SPIS_1_TX_INIT_INTERRUPTS_MASK;
    #endif /* SPIS_1_TXBUFFERSIZE > 4u */
...
void SPIS_1_WriteTxData(uint8 txData) 
{
        ...
        do
        {
            tmpTxBufferRead = SPIS_1_txBufferRead - 1u;
            if (tmpTxBufferRead < 0u)
            {
                tmpTxBufferRead = SPIS_1_TXBUFFERSIZE - 1u;
            }
        ...
        if((SPIS_1_txBufferRead == SPIS_1_txBufferWrite) &&
            ((SPIS_1_swStatusTx & SPIS_1_STS_TX_FIFO_NOT_FULL) != 0u))
        {
        ...
        else
        {
            ...
            if(SPIS_1_txBufferWrite >= SPIS_1_TXBUFFERSIZE)
            {
                SPIS_1_txBufferWrite = 0u;
            }
            ...
            {
                SPIS_1_txBufferRead++;
                if(SPIS_1_txBufferRead >= SPIS_1_RXBUFFERSIZE)
                {
                    SPIS_1_txBufferRead = 0u;
                }
        ...
        while((SPIS_1_TX_STATUS_REG & SPIS_1_STS_TX_FIFO_NOT_FULL) == 0u);
...
uint8 SPIS_1_GetRxBufferSize(void) 
{
    uint8 size = 0u;
        ...
        {
            size = (SPIS_1_RXBUFFERSIZE - SPIS_1_rxBufferRead) + SPIS_1_rxBufferWrite;
        }
        ...
        size = ((SPIS_1_RX_STATUS_REG & SPIS_1_STS_RX_FIFO_NOT_EMPTY) == 
                 SPIS_1_STS_RX_FIFO_NOT_EMPTY) ? 1u : 0u;
...
uint8  SPIS_1_GetTxBufferSize(void) 
{
    uint8 size = 0u;
        ...
        {
            size = (SPIS_1_TXBUFFERSIZE - SPIS_1_txBufferRead) + SPIS_1_txBufferWrite;
        }
        ...
        /* Is the fifo is full. */
        if((size & SPIS_1_STS_TX_FIFO_EMPTY) == SPIS_1_STS_TX_FIFO_EMPTY)
        {
            size = 0u;
        }
        else if((size & SPIS_1_STS_TX_FIFO_NOT_FULL) == SPIS_1_STS_TX_FIFO_NOT_FULL)
        {
            size = 1u;
        }
        else
...
void SPIS_1_ClearTxBuffer(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        /* Clear TX FIFO */
        SPIS_1_TX_AUX_CONTROL_DP0_REG |= SPIS_1_FIFO_CLR;
        SPIS_1_TX_AUX_CONTROL_DP0_REG &= ~SPIS_1_FIFO_CLR;
        ...
        SPIS_1_TX_AUX_CONTROL_DP0_REG |= SPIS_1_FIFO_CLR;
        SPIS_1_TX_AUX_CONTROL_DP0_REG &= ~SPIS_1_FIFO_CLR;
        SPIS_1_TX_AUX_CONTROL_DP1_REG |= SPIS_1_FIFO_CLR;
        SPIS_1_TX_AUX_CONTROL_DP1_REG &= ~SPIS_1_FIFO_CLR;
...
uint8 SPIS_1_ReadStatus(void) 
{
    ...
    #if ((SPIS_1_TXBUFFERSIZE > 4u) || (SPIS_1_RXBUFFERSIZE > 4u))

        SPIS_1_DisableInt();

        tmpStatus = SPIS_1_GET_STATUS_TX(SPIS_1_swStatusTx) |
                    (SPIS_1_GET_STATUS_RX(SPIS_1_swStatusRx) & ~SPIS_1_STS_RX_FIFO_FULL);
    ...
    #else /* (SPIS_1_TXBUFFERSIZE < 4u) && (SPIS_1_RXBUFFERSIZE < 4u) */

        tmpStatus = SPIS_1_TX_STATUS_REG | (SPIS_1_RX_STATUS_REG &
                                                      ~SPIS_1_STS_RX_FIFO_FULL);

    #endif /* (SPIS_1_TXBUFFERSIZE > 4u) || (SPIS_1_RXBUFFERSIZE > 4u) */
