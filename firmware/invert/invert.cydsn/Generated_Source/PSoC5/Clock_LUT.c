...
*
*******************************************************************************/
void Clock_LUT_Start(void) 
{
    /* Set the bit to enable the clock. */
    Clock_LUT_CLKEN |= Clock_LUT_CLKEN_MASK;
	Clock_LUT_CLKSTBY |= Clock_LUT_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_LUT_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Clock_LUT_CLKEN &= (uint8)(~Clock_LUT_CLKEN_MASK);
	Clock_LUT_CLKSTBY &= (uint8)(~Clock_LUT_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_LUT_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_StopBlock(void) 
{
    if ((Clock_LUT_CLKEN & Clock_LUT_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_LUT__CFG3)
        CLK_DIST_AMASK = Clock_LUT_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_LUT_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_LUT__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_LUT_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_LUT_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_LUT_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_LUT_CLKSTBY &= (uint8)(~Clock_LUT_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_LUT_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_LUT_GetSourceRegister();
    uint16 oldDivider = Clock_LUT_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_LUT_CLKEN & Clock_LUT_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_LUT_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_LUT_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_LUT_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_LUT_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_LUT_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_SetModeRegister(uint8 modeBitMask) 
{
    Clock_LUT_MOD_SRC |= modeBitMask & (uint8)Clock_LUT_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_LUT_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_LUT_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_LUT_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_LUT_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_LUT_GetModeRegister(void) 
{
    return Clock_LUT_MOD_SRC & (uint8)(Clock_LUT_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_LUT_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_LUT_GetDividerRegister();
    uint8 oldSrc = Clock_LUT_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_LUT_MOD_SRC |= CYCLK_SSS;
        Clock_LUT_MOD_SRC =
            (Clock_LUT_MOD_SRC & (uint8)(~Clock_LUT_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_LUT_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_LUT_GetSourceRegister(void) 
{
    return Clock_LUT_MOD_SRC & Clock_LUT_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_LUT_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_LUT_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_LUT_PHASE = clkPhase & Clock_LUT_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_LUT_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_LUT_GetPhaseRegister(void) 
{
    return Clock_LUT_PHASE & Clock_LUT_PHASE_MASK;
}
